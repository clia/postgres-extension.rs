...
pub fn expand_pg_export(cx: &mut ExtCtxt, span: Span, _: &MetaItem, item: Annotatable) -> Annotatable {
    ...
    match item {
        Annotatable::Item(it) => {
            ...
            new_it.attrs.push(attr::mk_attr_outer(span, attr::mk_attr_id(), attr::mk_word_item(Ident::from_str("no_mangle"))));
        ...
        Annotatable::ForeignItem(tt) => {
            let mut new_it = (*tt).clone();
            new_it.attrs.push(attr::mk_attr_outer(span, attr::mk_attr_id(), attr::mk_word_item(Ident::from_str("no_mangle"))));
            Annotatable::ForeignItem(P(new_it))
        }
        Annotatable::Stmt(tt) => {
            let mut new_it = (*tt).clone();
            //new_it.attrs.push(attr::mk_attr_outer(span, attr::mk_attr_id(), attr::mk_word_item(Ident::from_str("no_mangle"))));
            Annotatable::Stmt(P(new_it))
        }
        Annotatable::Expr(tt) => {
            let mut new_it = (*tt).clone();
            //new_it.attrs.push(attr::mk_attr_outer(span, attr::mk_attr_id(), attr::mk_word_item(Ident::from_str("no_mangle"))));
            Annotatable::Expr(P(new_it))
        }
    }
